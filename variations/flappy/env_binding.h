...
// Python function to reset the environment (difficulty optional; default 0.0)
static PyObject* env_reset(PyObject* self, PyObject* args) {
    if (PyTuple_Size(args) != 2 && PyTuple_Size(args) != 3) {
        PyErr_SetString(PyExc_TypeError, "env_reset requires 2 (env, seed) or 3 (env, seed, difficulty) arguments");
        return NULL;
    }
    ...
    if (PyTuple_Size(args) == 3) {
        PyObject* diff_arg = PyTuple_GetItem(args, 2);
        if (PyFloat_Check(diff_arg))
            difficulty = (float)PyFloat_AsDouble(diff_arg);
        else if (PyLong_Check(diff_arg))
            difficulty = (float)PyLong_AsLong(diff_arg);
        else {
            PyErr_SetString(PyExc_TypeError, "difficulty must be a number");
            return NULL;
        }
    }
    c_reset(env, difficulty);
    Py_RETURN_NONE;
}
...
static PyObject* vec_reset(PyObject* self, PyObject* args) {
    if (PyTuple_Size(args) != 2 && PyTuple_Size(args) != 3) {
        PyErr_SetString(PyExc_TypeError, "vec_reset requires 2 (vec, seed) or 3 (vec, seed, difficulty) arguments");
        return NULL;
    }
...
static PyObject* vec_step(PyObject* self, PyObject* arg) {
    ...
    if (num_args != 1 && num_args != 2) {
        PyErr_SetString(PyExc_TypeError, "vec_step requires 1 or 2 (vec, phase) arguments");
        return NULL;
    }
    ...
    /* If a difficulty argument is provided, push it into every sub-env
       so that auto-resets inside c_step use the up-to-date difficulty. */
    if (num_args == 2) {
        PyObject* diff_arg = PyTuple_GetItem(arg, 1);
        float difficulty;
        if (PyFloat_Check(diff_arg))
            difficulty = (float)PyFloat_AsDouble(diff_arg);
        else if (PyLong_Check(diff_arg))
        ...
        for (int i = 0; i < vec->num_envs; i++) {
            vec->envs[i]->curriculum_difficulty = difficulty;
        }
    }
